...
SELECT     name,
           ...
           CASE WHEN lower(name) LIKE '%omop%' OR lower(name) LIKE 'omop%' OR split_part(name,'.', -1) = 'csv' OR split_part(name,'.', -1) = 'xlsx' THEN 'OMOP'
                ...
                WHEN lower(name) LIKE '%note%' OR lower(name) LIKE 'note%' THEN 'NOTE'
           END AS mode,
...
WITH  bs_tmp AS (
    ...
    SELECT container,
           ...
           COALESCE(ROUND(((IMAGE_SIZE / TOTAL_SIZE) * 100)::numeric, 2), 0) AS NOTE_REL,
           MOST_RECENT_OMOP,
           MOST_RECENT_WAVE,
           MOST_RECENT_IMAGE,
           MOST_RECENT_NOTE,
