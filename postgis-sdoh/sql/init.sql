...
(
    ...
    geom_type              varchar(100) NULL, -- Note - I left out geom_spec and using shp2pgsql to load shp files
    ...
    srid                   integer NULL, -- Added SRID for shape file loading
    geom_join_column       varchar(255) NULL
);
--HINT DISTRIBUTE ON RANDOM
CREATE TABLE backbone.variable_source
(
    variable_source_id   serial4,
    geom_dependency_uuid int4 NULL,
    variable_name        varchar NULL,
    variable_desc        text    NULL,
    data_source_uuid     int4    NULL,
    geom_join_column	 text    NULL, -- Note - I expanded attr_spec into columns for reference in sql logic
    attr_concept_id		 text    NULL,
    attr_start_date		 text    NULL,
    attr_end_date		 text    NULL,
    attr_source_value	 text    NULL,
...
select row_number()                                        over() as geom_index_id
     ...
     , regexp_replace(regexp_replace(lower(concat(org_id, '_', org_set_id)), '\W', '_', 'g'), '^_+|_+$|_(?=_)', '',
                      'g')                              as database_schema
     , regexp_replace(regexp_replace(lower(concat(dataset_name)), '\W', '_', 'g'), '^_+|_+$|_(?=_)', '',
                      'g')                              as table_name
...
where geom_type <> ''
  ...
  and data_source_uuid not in (select data_source_uuid
                               from geom_index);
...
select row_number()           over() as attr_index_id
     ...
     , regexp_replace(regexp_replace(lower(concat(ds.org_id, '_', ds.org_set_id)), '\W', '_', 'g'), '^_+|_+$|_(?=_)',
                      '',
                      'g') as database_schema
     , regexp_replace(regexp_replace(lower(concat(ds.dataset_name)), '\W', '_', 'g'), '^_+|_+$|_(?=_)', '',
                      'g') as table_name
...
-- WHERE vocabulary_id NOT IN (SELECT vocabulary_id
...
-- WHERE concept_class_id NOT IN (SELECT concept_class_id
...
-- WHERE domain_id NOT IN (SELECT domain_id
...
-- INSERT INTO vocabulary.concept_relationship
-- select rev.*
-- from (select cr.concept_id_2           as concept_id_1
...
--                               and cr.concept_id_1 > 2000000000) rev
--          left join (select *
--                     from vocabulary.concept_relationship
--                     where concept_id_1 > 2000000000) orig
...
CREATE TABLE working.location_history
(
    location_id                  integer,
...
CREATE TABLE working.external_exposure
(
