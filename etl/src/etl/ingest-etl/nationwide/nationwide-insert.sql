...
SELECT
    co.condition_occurrence_id::bigint,
    co.person_id::bigint,
    co.condition_concept_id::bigint,
    ...
    co.condition_type_concept_id::bigint,
    co.condition_status_concept_id::bigint,
    co.stop_reason,
    co.provider_id::bigint,
    co.visit_occurrence_id::bigint,
    NULL AS visit_detail_id,
    -- co.visit_detail_id::bigint,
    co.condition_source_value AS condition_source_value,
    CASE WHEN co.condition_source_concept_id::bigint = 0 THEN NULL
        ELSE co.condition_source_concept_id::bigint
    END AS condition_source_concept_id,
    condition_status_source_value
FROM
    omopcdm.src_condition_occurrence AS co
    JOIN persist.date_shift AS ds ON co.person_id::bigint = ds.person_id;
...
SELECT
    ...
    unique_device_id,
    de.production_id::bigint,
    de.quantity::bigint,
    de.provider_id::bigint,
    de.visit_occurrence_id::bigint,
    NULL AS visit_detail_id,
    --de.visit_detail_id::bigint,
    device_source_value,
    CASE WHEN de.device_source_concept_id::bigint = 0 THEN NULL
        ELSE de.device_source_concept_id::bigint
...
FROM
    ...
    JOIN persist.date_shift AS ds ON de.person_id::bigint = ds.person_id;

INSERT INTO
    omopcdm.drug_exposure
SELECT
    de.drug_exposure_id::bigint,
    de.person_id::bigint,
    de.drug_concept_id::bigint,
    ...
    de.drug_type_concept_id::bigint,
    de.stop_reason,
    de.refills::float::integer,
    de.quantity::float,
    de.days_supply::float::integer,
    de.sig,
    de.route_concept_id::bigint,
    de.lot_number,
    de.provider_id::bigint,
    de.visit_occurrence_id::bigint,
    NULL AS visit_detail_id,
    --de.visit_detail_id::bigint,
    drug_source_value,
    CASE WHEN de.drug_source_concept_id::bigint = 0 THEN NULL
        ELSE de.drug_source_concept_id::bigint
...
SELECT
    ...
    CASE WHEN m.measurement_source_concept_id::bigint = 0 THEN NULL
        ELSE m.measurement_source_concept_id::bigint
...
FROM
    ...
    JOIN persist.date_shift AS ds ON m.person_id::bigint = ds.person_id;
...
SELECT
    ...
    CASE WHEN o.observation_source_concept_id::bigint = 0 THEN NULL
        ELSE o.observation_source_concept_id::bigint
    END AS observation_source_concept_id,
    o.unit_source_value,
    o.qualifier_source_value,
    o.value_source_value,
    NULL AS observation_event_id,
    --o.observation_event_id::bigint,
    NULL AS obs_event_field_concept_id
    --o.obs_event_field_concept_id::bigint
FROM
    omopcdm.src_observation AS o
    JOIN persist.date_shift AS ds ON o.person_id::bigint = ds.person_id;

INSERT INTO
    omopcdm.observation_period
SELECT
    op.observation_period_id::bigint,
    op.person_id::bigint,
    ...
    op.period_type_concept_id::bigint
FROM omopcdm.src_observation_period AS op
JOIN persist.date_shift AS ds ON op.person_id::bigint = ds.person_id;
...
WITH
    person_shifted AS (
        ...
        JOIN persist.date_shift AS ds ON p.person_id::bigint = ds.person_id
    )
INSERT INTO
    omopcdm.person
SELECT
...
SELECT
    po.procedure_occurrence_id::bigint,
    po.person_id::bigint,
    po.procedure_concept_id::bigint,
    ...
    po.procedure_type_concept_id::bigint,
    po.modifier_concept_id::bigint,
    po.quantity::integer,
    po.provider_id::bigint,
    po.visit_occurrence_id::bigint,
    NULL AS visit_detail_id,
    -- po.visit_detail_id::bigint,
    po.procedure_source_value,
    CASE WHEN po.procedure_source_concept_id::bigint = 0 THEN NULL
        ELSE po.procedure_source_concept_id::bigint
    END AS procedure_source_concept_id,
    po.modifier_source_value
FROM
    omopcdm.src_procedure_occurrence AS po
    JOIN persist.date_shift AS ds ON po.person_id::bigint = ds.person_id;
...
SELECT
    ...
    COALESCE(vd.visit_detail_end_date::date, vd.visit_detail_start_date::date) + INTERVAL'1 day'*ds.days AS visit_detail_end_date,
...
FROM
    ...
    JOIN persist.date_shift AS ds ON vd.person_id::bigint = ds.person_id;
...
SELECT
    ...
    COALESCE(vo.visit_end_date::date, vo.visit_start_date::date) + INTERVAL'1 day'*ds.days AS visit_end_date,
...
FROM
    ...
    JOIN persist.date_shift AS ds ON vo.person_id::bigint = ds.person_id;
...
SELECT
    'NATIONWIDE' AS cdm_source_name,
    'NATIONWIDE' AS cdm_source_abbreviation,
    'NATIONWIDE' AS cdm_holder,
